...
class IbarakiSpider(AbstractSpider):
    ...
    def parse(self, response):
        ...
        for article in response.xpath(
            ...
            # MEMO: 詳細ページまで見れば「公式URL」「定休日」「営業時間」が取れるが、企業サービスを過剰に叩きたくないので今回はパス
            # といっても同じ推理の絆を使ってる三重県、岐阜県では(仕方なく)詳細ページまで叩いてるので、今更感はある…

            yield item

        # リンクボタンがなければ(最終ページなので)終了
        next_page = response.xpath(
            f'//table[@class="smp-pager"]//td[@class="smp-page smp-current-page"]/following-sibling::td[1]/a/@href'
        ).extract_first()
