...
class ShizuokaBlueSpider(AbstractSpider):
    ...
    """
    name = 'shizuoka_blue'
    allowed_domains = [ 'gotoeat-shizuoka.com' ] # .comとは
    ...
    def parse(self, response):
        ...
        for article in response.xpath('//ul[@id="shop_list"]/li[@class="shop_box"]'):
            ...
            address = article.xpath('.//dl[@class="accordion"]//table/tr/th[contains(text(), "住所")]/following-sibling::td/text()').getall()
            item['address'] = ''.join([w3lib.html.remove_tags(x.strip()) for x in address])

            item['tel'] = article.xpath('.//dl[@class="accordion"]//table/tr/th[contains(text(), "電話番号")]/following-sibling::td/text()').get()
            item['opening_hours'] = article.xpath('.//dl[@class="accordion"]//table/tr/th[contains(text(), "営業時間")]/following-sibling::td/text()').get()
