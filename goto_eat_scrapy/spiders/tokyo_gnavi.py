...
class TokyoGnaviSpider(AbstractSpider):
    ...
    def parse(self, response):
        ...
        for article in response.xpath(
            '//div[@class="result-cassette__wrapper result-cassette__wrapper--normal"]/ul[@class="result-cassette__list"]/li'
        ):
            url = article.xpath(
                './/div[@class="result-cassette__box"]//a[@class="result-cassette__box-title js-measure"]/@href'
            ).get()
            yield scrapy.Request(url, callback=self.detail)

        # 「>」ボタンがなければ(最終ページなので)終了
        next_page = response.xpath(
            '//nav//li[@class="pagination__arrow-item"]/a[@class="pagination__arrow-item-inner pagination__arrow-item-inner-next"]/@href'
        ).extract_first()
    ...
    def detail(self, response):
        ...
        item["area_name"] = response.xpath(
            '//ol[@id="gn_info-breadcrumbs-inner"]/li/a[contains(text(), "東京")]/../following-sibling::li/a/text()'
        ).extract_first()
        for tr in response.xpath('//div[@id="info-table"]/table/tbody'):
            item["shop_name"] = (
                tr.xpath('.//tr/th[contains(text(), "店名")]/following-sibling::td/p[@id="info-name"]/text()')
                .get()
                .strip()
            )
            item["tel"] = tr.xpath(
                './/tr/th[contains(text(), "電話番号・FAX")]/following-sibling::td/ul/li/span[@class="number"]/text()'
            ).get()

            # data-oに入ってる謎JSON？をparseしてURLを組み立て
            data_o = tr.xpath(
                './/tr/th[contains(text(), "お店のホームページ")]/following-sibling::td/ul/li/a[@class="url go-off"]/@data-o'
            ).get()
            if data_o:
            ...
            zip_code = tr.xpath(
                './/tr/th[contains(text(), "住所")]/following-sibling::td/p[@class="adr slink"]/text()'
            ).get()
            item["zip_code"] = zip_code.strip().replace("〒", "") if zip_code else None
            item["address"] = (
                tr.xpath(
                    './/tr/th[contains(text(), "住所")]/following-sibling::td/p[@class="adr slink"]/span[@class="region"]/text()'
                )
                .get()
                .strip()
            )
        ...
        if genre_list:
            ...
            item["genre_name"] = (
                response.xpath('//header[@role="banner"]//dd[@id="header-meta-cat-desc"]/text()').get().strip()
            )
