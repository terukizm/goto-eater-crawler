...
class KagoshimaSpider(AbstractSpider):
    ...
    # FIXME: 鹿児島は2020/12/04？にpdfをやめてhtmlにしてくれたが、おそらく元データのExcelからフィルタで行を非表示にしたものを
    # ExcelのWebページ発行ウィザードで出力しているだけなので、HTMLソースを見ると重複した内容が含まれている。
    # 件数から見ておそらく「鹿児島市全域」と「その他地域」で２つのファイルがあり、それを出し分けしてると思うので、
    ...
    def parse(self, response):
        for p in response.xpath('//div[@id="contents_layer"]/span/p'):
            ...
            if area_name in self.area_list:
                ...
                # MEMO: 暫定的にExcelのWebページ発行ウィザードの仕様に合わせてコメントアウト。最後まで本気でこれで行きそうだったら実装を直すが、
    ...
    def parse_from_area_html(self, response):
        ...
        for article in response.xpath('//table/tr'):
            if article.xpath('.//td[2]/a/span[contains(text(), "検索")]').get():
                ...
                # (実質2データしかなく、それらをdisplay:noneで出し分けしており、
                # おそらく一つのExcelシートにフィルタをかけて行を非表示にしたりして出し分けしている)
                # 最後まで本気でこのWebページ形式で行く感じだったら、諦めて対応する…かも
