...
class KyotoSpider(AbstractSpider):
    ...
    def detail(self, response):
        ...
        item["genre_name"] = (
            article.xpath('.//div[@class="store-cont"]/table/tr/th[contains(text(), "ジャンル")]/following-sibling::td/text()').get().strip()
        )
        item["area_name"] = (
            article.xpath('.//div[@class="store-cont"]/table/tr/th[contains(text(), "エリア")]/following-sibling::td/text()').get().strip()
        )
        item["address"] = (
            article.xpath('.//div[@class="store-cont"]/table/tr/th[contains(text(), "住所")]/following-sibling::td/text()').get().strip()
        )
        item["tel"] = (
            article.xpath('.//div[@class="store-cont"]/table/tr/th[contains(text(), "電話番号")]/following-sibling::td/text()').get().strip()
        )
        # MEMO: 詳細ページに項目自体はあるが、電話番号、定休日が入ってるデータは1件もない(2021/01/18)
        item["opening_hours"] = (
            article.xpath('.//div[@class="store-cont"]/table/tr/th[contains(text(), "営業時間")]/following-sibling::td/text()').get()
        )
        item["closing_day"] = (
            article.xpath('.//div[@class="store-cont"]/table/tr/th[contains(text(), "定休日")]/following-sibling::td/text()').get()
        )
