...
class GotoEatScrapyPipeline:
    ...
    def _normalize(self, item, spider):
        ...
        # MEMO: 推理の絆系のシステムで、official_page(URL)中にHTMLタグが含まれるのがいくつかあるが、とりあえず補正は見送り
    ...
    # MEMO: エラーチェックはcsv2geojson側で行う
    # def _validate(self, item):
    #     # 必須チェック
    #     for attr in ['shop_name', 'address']:
    #         if not item.get(attr):
    #             raise ScrapingError(f'{attr}は必須です。')
    #     # 書式エラー(未入力を許可)
    #     # (MEMO: DropItemExceptionが発生した場合、クローリング処理自体は止まらず該当データだけが落ちる)
    #     if (tel := item.get('tel')) and not re.match(r'^0\d{9,10}$', str(tel).strip().replace('-', '')):
    ...
    #     if (zip_code := item.get('zip_code')) and not re.match(r'^\d{7}$', re.sub('--‐　', '', str(zip_code))):
